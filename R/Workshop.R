...
#3-way crosstab
table(students$Level, students$Division, students$Military)
as.data.frame(table(students$Level, students$Division, students$Military))
#The prop.table will give a proportion table
prop.table(table(students$Division, students$Military))
prop.table(table(students$Division, students$Military)) * 100
...
table(students$Level, useNA='always')
...
tmp = head(students$Enrolled)
format(tmp, format='%A, %B %d, %Y')
#Look at the lubridate package for more on dates
today = Sys.Date()
today - tmp
(today - tmp) / 365.25
floor( (today - tmp) / 365.25 )
ceiling( (today - tmp) / 365.25 )
round( (today - tmp) / 365.25 )
...
ggplot(students, aes(x=Credits)) + geom_histogram()
...
table(combinedSurvey$Question1)
table(combinedSurvey$Administration, combinedSurvey$Question1)

duplicated(combinedSurvey$StudentID)
duplicated(combinedSurvey$StudentID, fromLast=TRUE)
...
merge(combinedSurvey, students, by.x=c('StudentID'), by.y=c('StdID'), all.x=TRUE, all.y=TRUE)
...
directory = getIPEDSSurvey('HD', 2010)
directory = recodeDirectory(directory)
directory[directory$instnm == 'Excelsior College',]
...
table(as.character(ret2$UseAdmissionTestScores))
...
head(ret2[,c('SATWriting25','SATWriting75', 'SATWriting')])
...
head(ret2[,c('SATTotal', 'FullTimeRetentionRate', 'NumSATScores', 'UseAdmissionTestScores')])
ggplot(ret2, aes(x=SATTotal, y=FullTimeRetentionRate, size=NumSATScores, 
				 color=UseAdmissionTestScores)) + geom_point()

ggplot(ret2, aes(x=SATTotal, y=FullTimeRetentionRate, size=NumSATScores)) + 
	geom_smooth() + geom_point(alpha=.2)
